...
IF OBJECT_ID('dbo.usp_mw_ww_load_inbound_failed_message_view') IS NOT NULL
    DROP PROCEDURE [dbo].[usp_mw_ww_load_inbound_failed_message_view];
GO
...
CREATE PROCEDURE [dbo].[usp_mw_ww_load_inbound_failed_message_view]
...
DECLARE
    ...
    -- Set Constants
    SET @c_vchObjName = N'usp_mw_ww_load_inbound_failed_message_view';
...
BEGIN TRY
    ...
    DELETE FROM t_mw_ww_view_inbound_failed_message
...
BEGIN CATCH
    SET @v_nSysErrorNum = ERROR_NUMBER();
    SET @v_vchCode = ERROR_LINE();
    SET @v_vchMsg = N'An error occurred while deleting from t_mw_ww_view_inbound_failed_message.';
    SET @v_vchMsg = @v_vchMsg + N' SQL Error = ' + ERROR_MESSAGE();
    GOTO ERROR_HANDLER;
END CATCH
...
BEGIN TRY
    ...
    INSERT INTO t_mw_ww_view_inbound_failed_message (
        ww_user_id,
        view_id,
        mw_inbound_failed_message_id
    )
    SELECT @v_vchWwUserId
          ,@v_vchViewId
          ,mw_inbound_failed_message_id
      FROM t_mw_inbound_failed_message
     WHERE status IN (''N'', ''E'')
...
BEGIN TRY
    ...
    IF @in_vchMwInboundId <> '%'
        SET @v_vchSqlQuery = @v_vchSqlQuery + ' AND mw_inbound_id LIKE @v_vchMwInboundId';
