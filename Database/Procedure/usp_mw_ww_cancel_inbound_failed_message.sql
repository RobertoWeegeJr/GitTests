...
IF OBJECT_ID('dbo.usp_mw_ww_cancel_inbound_failed_message') IS NOT NULL
    DROP PROCEDURE [dbo].[usp_mw_ww_cancel_inbound_failed_message];
GO
...
CREATE PROCEDURE [dbo].[usp_mw_ww_cancel_inbound_failed_message]
...
DECLARE @t_cancelled_messages TABLE (
    ...
    -- Set Constants
    SET @c_vchObjName = N'usp_mw_ww_cancel_inbound_failed_message';
...
BEGIN TRY
    ...
    UPDATE ifm
       SET ifm.status = 'X'
    OUTPUT INSERTED.mw_inbound_failed_message_id
          ,INSERTED.mw_inbound_id
...
BEGIN TRY
    ...
    SELECT @v_vchTranGroupID
          ...
          ,cmg.mw_inbound_failed_message_id                   AS control_number
          ,cmg.mw_inbound_id                                  AS generic_text1
      ...
      LEFT JOIN t_whse_mw_inbound wmi
        ON wmi.mw_inbound_id = cmg.mw_inbound_id
      LEFT JOIN t_transaction trn
             ON trn.system_id = 'WA'
            AND trn.tran_type = '731';
