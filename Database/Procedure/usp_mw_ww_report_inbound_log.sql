...
IF OBJECT_ID('dbo.usp_mw_ww_report_inbound_log') IS NOT NULL
    DROP PROCEDURE [dbo].[usp_mw_ww_report_inbound_log];
GO
...
CREATE PROCEDURE [dbo].[usp_mw_ww_report_inbound_log]
...
DECLARE
    ...
    -- Set Constants
    SET @c_vchObjName = N'usp_mw_ww_report_inbound_log';
...
BEGIN TRY
    ...
    SELECT mw_inbound_log_id, record_created_date, record_created_datetime, mw_inbound_id, sssb_conversation_handle, message_type, order_type, message_id, message
       FROM t_mw_inbound_log
...
BEGIN TRY
    ...
    IF @in_vchSssbConversationHandle <> '%'
        SET @v_vchSqlQuery = @v_vchSqlQuery + ' AND CAST(sssb_conversation_handle AS NVARCHAR(200)) LIKE @v_vchSssbConversationHandle';
